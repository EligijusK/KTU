...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        //view.aStarPathfind(cellWeights, bot)
                    ...
                    cells(i) match {
                      ...
                      case 'b' => // bad beast: dangerous, but only if very close
                        for (j <- 0 until cellCount) {
                          ...
                          val stepDistance = cellRelPos.stepsTo(pos)
                          if (pos.isNonZero && stepDistance != 0) {
                            if (stepDistance < 4) cellWeights(j) += 4000 / stepDistance
                            else cellWeights(j) += 500 / stepDistance
                          }
                        }
            ...
            if (cells.contains('P') || cells.contains('B')) {
                ...
                if(found)
                {
                        ...
                        {
                            index = path_index(path_index.size - 1)
        ...
        (index, nearestEnemyMaster, nearestEnemySlave)
    }
                        ...
                        if((x != 0 && y != 0) ||(x == 0 && y != 0) || (x != 0 && y == 0))
                        {
                            ...
                            { 
                                if(pos == destination)
                                {
                                    ...
                                    var (path, path_index) = tracePath(parent, parent_coordinates, pos, startingPoint, view, bot)
                                    foundDest = true
                                    break
                                }

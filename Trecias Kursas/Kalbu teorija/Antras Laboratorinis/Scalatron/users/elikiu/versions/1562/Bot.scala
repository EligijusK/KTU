...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        // this for creates heatmap for helping a* algorith find road to position with weights
            ...
            val previuosStepCount = bot.inputAsIntOrElse("lastCount", 0).toInt
            if ((cells.contains('P') || cells.contains('B')) || (lastCount < 1 && lastCount > 2)) {
                ...
                if(!closestFood.isZero)
                {
                   ...
                   var (temp_closestFood) = findClosestThings(cellCount, cells, view, cellWeights, bot)
        ...
        def findClosestThings(cellCount: Int, cells: String, view: View, weights: Array[Double], bot: Bot): (XY) = {
        ...
        breakable{
            while(!open_list.isEmpty)
            {
                ...
                for(x <- -1 to 1)
                {
                    for(y <- -1 to 1)
                    {
                        // prevent form adding zero coordinates
                        if((x != 0 && y != 0) ||(x == 0 && y != 0) || (x != 0 && y == 0))
                        {
                            ...
                            { 
                                ...
                                else if(closed_list(index) == false && isUnBlocked(cells, index)) // check if element is not blocked and coordinates ins not used
                                {
                                    // calculate vaerage weight around new element. this helps decide new coordinates with geat map
                                    ...
                                    // calculating weight for coordinates
                                    var gNew = 0.0
                                    if((x == 0 && y != 0) || (x != 0 && y == 0))
                                    {
                                    ...
                                    // check if use this coordinates or more efficent coordinates is already selected
                                    if (f(index) == Float.MaxValue || f(index) > fNew) 
                                    { 
                                        // add new coordinates for next coordinates
                                        open_list = open_list :+ pos
                                        open_list_f = open_list_f :+ fNew
                                        //bot.log(index.toString)
                                        // update all values fom init to real values
    ...
    def tracePath(parent: ListBuffer[Int], parent_coordinates: ListBuffer[XY], last: XY, dest: XY, view: View, bot: Bot) = { 
     ...
     // almoust the same as master bot changed target search
     def analyzeViewAsBot(bot: Bot, masterVal: Int) = {
        ...
        val indexRel = view.indexFromRelPos(XY(0,0))
                ...
                // this for creates heatmap for helping a* algorith find road to position with weight. When heatmap ia updating closest enemy is updating too
                for(i <- 0 until cellCount) {
                  val cellRelPos = view.relPosFromIndex(i)
                  if(cellRelPos.isNonZero && !view.outOfBoundsRel(cellRelPos)) {
                    cells(i) match {
            ...
            if ((cells.contains('m') || cells.contains('s')) || (lastCount < 1 && lastCount > 2)) {
                ...
                {   
                    ...
                    if(!bot.view.outOfBoundsRel(nearestEnemy) && !nearestEnemy.isZero)
                    {
                        ...
                        if(found)
                        {
                                // take first coordinate and calculate move direction
                                if(path(path.size - 1) != XY(0,0))
                                {
            ...
            else if((!cells.contains('m') && !cells.contains('s')) || (lastCount > 0 && lastCount < 3)) // use refrence algorithm if enemy is not in view or path by a* is not found
            {
            ...
            if((lastCount == 0 || lastCount > 2) && direction45 == lastDirection) // if bot last direction is same as next direction
            {
                bot.set("lastCount" -> 1.toString)
            }
        ...
        // almoust the same as master bot target changes if harvest bot have 1500 energy adn is vissable by bot

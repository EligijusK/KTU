...
object ControlFunction
{
    def forMaster(bot: Bot) {
        ...
        if(dontPlantMineUntil < bot.time && bot.energy > 1000){
    ...
    def reactAsHarvest(bot: MiniBot) {
            ...
            val (directionValue, nearestEnemyMaster, nearestEnemySlave) = = bot.analyzeViewAsMaster(bot.view)
            bot.move(directionValue)
            bot.set("lastDirection" -> directionValue.toDirection45)
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        var weightsForFood = ListBuffer[Double]()
        for (i <- 0 until cellCount) {
            val cellRelPos = view.relPosFromIndex(i)
                if(cellRelPos.isNonZero && !view.outOfBoundsRel(cellRelPos)) {
                    cells(i) match {
                        ...
                        case 'P' => // another master: not dangerous, but an obstacle
                            indexes = indexes :+ i
                            var testWeight = 0.0
                            var cnt = 0
                            for (x <- -4 to 4) {
                        ...
                        case 'B' => // another master: not dangerous, but an obstacle
                            indexes = indexes :+ i
                            var testWeight = 0.0
                            var cnt = 0
                            for (x <- -4 to 4) {
   ...
   /* def findFarestThing(cellCount: Int, cells: String, view: View, bot: Bot, weights: Array[Double]): (XY) = {

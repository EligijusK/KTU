...
object ControlFunction
{
    ...
    def findClosestThings(cellCount: Integer, cells: String, view: View): (XY, Option[XY], Option[XY], Integer) = {
        ...
        for (i <- 0 until cellCount) {
            ...
            if (cells(i) == 's' || cells(i) == 'b' || cells(i) == 'm') {
                badGuysCount += 1
            }
        }
...
case class View(cells: String) {
    ...
    def aStarPathfind(cellWeights : Array[Double], bot: Bot) = {
        var Q = cells.indices.toList    //Queue cells
        var CloseQ = List[String]() 
        var g = Array.ofDim[Double](cellCount)
        var f = Array.ofDim[Double](cellCount)
        var h = Array.ofDim[Double](cellCount)
        var start = indexFromAbsPos(center)
        var steps = Array.ofDim[Int](cellCount)     //Zingsniai reikalimngi pasiekti cell
        
        
        // is cia gauti atsumus aplinkui
        // atstumas iki visu kitu vietu
        for(i <- 0 until cellCount){
            h(i) = 10000 * relPosFromIndex(i).stepCount;
        }
        
        
        // atstumas kai stovi ant tos kordinates
        h(start) = 0
        
        // dist(indexFromAbsPos(center)) = 0

        while(Q.nonEmpty){
            
            // pizdins i geriausia vieta
            val u = h.view.zipWithIndex.map(_._2).filter(Q.contains(_)).minBy(h(_))
            val pos = absPosFromIndex(u)  //Pozicija galimo sekancio ejimo
            // istrina vieta kur eina
            //bot.log(Q.mkString(" "))
            Q = Q.filterNot(index => index == u)
            
            for(x <- -1 to 1){
                for(y <- -1 to 1){
                    val usrPos = center + XY(x,y)  
                    if(!outOfBoundsAbs(usrPos)){
                        val distance = pos - usrPos
                        bot.log("usrPos:" + distance)
                    }

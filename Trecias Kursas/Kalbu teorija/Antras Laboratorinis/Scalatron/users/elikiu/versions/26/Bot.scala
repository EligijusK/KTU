...
case class View(cells: String) {
    ...
    def distanceBeatweenStartEnd(start: Array[Int], end: Array[Int]) = {
        var distance = Array[Int](1)
    ...
    def aStarPathfind(cellWeights : Array[Double]) = {
        ...
        for(i <- 0 until cellCount){
            //dist(i) = 10000 * relPosFromIndex(i).stepCount;
        }

        while(Q.nonEmpty){
            val u = dist.view.zipWithIndex.map(_._2).filter(Q.contains(_)).minBy(dist(_))

...
object ControlFunction
{
        ...
        //view.aStarPathfind(cellWeights, bot)
            ...
            if (cells.contains("P") || cells.contains("B")) {
                 val (closestFood, innerMaster, innerSlave) = findClosestThings(cellCount, cells, view)
                  val path = aStarPathfind(cells, XY.Zero, view, closestFood)
    ...
    def aStarPathfind(cells: String, startingPoint: XY, view: View, destination: XY) = {
        ...
        var xTemp = startingPoint.x
        var yTemp = startingPoint.y
        ...
        for(x <- -1 to 1)
        {
            for(y <- -1 to 1)
            {
                open_list = open_list.filter( 1 < _)
                var pos = startingPoint + XY(x, y)
    ...
    def remove(num: Int, list: List[Int]) = list diff List(num)remove: (num: Int,list: List[Int])List[Int]

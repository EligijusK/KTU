...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        val indexRel = view.indexFromAbsPos(XY(0,0))
        cellWeights(indexRel) = 3333333
                ...
                for(i <- 0 until cellCount) {
                  ...
                  if(cellRelPos.isNonZero) {
                    ...
                    case 'b' =>
                        val pos = view.relPosFromIndex(i)
                        val stepDistance = cellRelPos.stepsTo(pos)
                        if (stepDistance == 1) cellWeights(i) += 200000
                        else if (stepDistance == 2) cellWeights(i) += 200000
                        else cellWeights(i) += 200000
        ...
        for (i <- 0 until cellCount) {
            if (cells(i) == 'P' || cells(i) == 'B') {
                ...
                if (cellRelPos.isNonZero) {
                    ...
                    if(testWeight < 1000000 && bot.energy > 2500)
                    {
                        distances(i) -= stepDistance / 2
                    }
                    if(testWeight < 500000 && bot.energy < 2500)
                    {
                        distances(i) -= stepDistance / 2
                    }

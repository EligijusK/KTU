...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        //view.aStarPathfind(cellWeights, bot)
            ...
            {
                    val centerIndex = view.indexFromAbsPos(view.center);
                    var maxValue = 0.0;
                    var bestIndex = centerIndex;

                   for(i <- 0 until cellCount) {
                      val cellRelPos = view.relPosFromIndex(i)
                      if(cellRelPos.isNonZero && cellDistances(i) < 10000) {
                        val stepDistance = steps(i)
                    ...
                    val direction = view.relPosFromIndex(i)

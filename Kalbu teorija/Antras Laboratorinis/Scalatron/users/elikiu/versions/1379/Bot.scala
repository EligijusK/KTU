...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        //view.aStarPathfind(cellWeights, bot)
            ...
            var direction45 = 0
            val lastDirection = bot.inputAsIntOrElse("lastDirection", 0)
            val lastCount = bot.inputAsIntOrElse("lastCount", 0)
            ...
            else if((!cells.contains('P') && !cells.contains('B')) || (lastCount > 0 && lastCount < 2))
            {
                ...
                bot.set("lastCount" -> lastCount.toInt + 1)
                    
            }
            if(lastCount == 0 && direction45 == lastDirection)
            {
                bot.set("lastCount" -> 1)
            }
            ...
            if (cells.contains('m')) {
                ...
                if(nearestEnemyMaster.isNonZero)
                {
                    ...
                    if(found)
                    {
                       ...
                       //var (temp_closestFood, temp_nearestEnemyMaster, temp_nearestEnemySlave) = findClosestThings(cellCount, cells, view, cellWeights, bot)
                        if(!bot.view.outOfBoundsRel(nearestEnemyMaster))
                        {
                            bot.log("PATH NOT FOUND")
                            //var (closestFood, nearestEnemyMaster, nearestEnemySlave) = findClosestThings(cellCount, cells, view, cellWeights, bot)
                            var (path, path_index, found) = aStarPathfind(cells, XY.Zero, view, nearestEnemyMaster, bot, cellWeights)

...
object ControlFunction
{
    def forMaster(bot: Bot) {
        ...
        val dontReleaseKamekazeUntil = bot.inputAsIntOrElse("dontReleaseKamekazeUntil", -1)
        ...
        if(dontReleaseHunterGathererUntil < bot.time && bot.energy > 100) { // fire defensive missile?
            nearestFood match {
                case None => // no-on nearby
                case Some(relPos) => // a master is nearby
                    if(relPos.stepCount < 12) {
                    val unitDelta = relPos.signum
                    bot.spawn(unitDelta, "mood" -> "HunterGatherer")
                    bot.set("dontReleaseKamekazeUntil" -> (bot.time + relPos.stepCount + 1))
                    }
            }
        }

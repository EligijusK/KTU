...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        //view.aStarPathfind(cellWeights, bot)
            ...
            if ((cells.contains('P') || cells.contains('B')) || (lastCount < 1 && lastCount > 3)) {
            ...
            else if((!cells.contains('P') && !cells.contains('B')) || (lastCount > 0 && lastCount < 4))
            {
            ...
            if((lastCount == 0 || lastCount > 3) && direction45 == lastDirection)
            {
                bot.set("lastCount" -> 1.toString)
            }

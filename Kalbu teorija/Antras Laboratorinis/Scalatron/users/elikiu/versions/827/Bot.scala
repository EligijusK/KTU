    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        //view.aStarPathfind(cellWeights, bot)
            ...
            if (cells.contains('P') || cells.contains('B')) {
                var (closestFood, nearestEnemyMaster, nearestEnemySlave) = findClosestThings(cellCount, cells, view, cellWeights)
        ...
        def findClosestThings(cellCount: Integer, cells: String, view: View, weights: Array[Double]): (XY, Option[XY], Option[XY]) = {
            ...
            if (cells(i) == 'P' || cells(i) == 'B') {
                ...
                for(x <- -1 to 1)
                {
                    for(y <- -1 to 1 )
                    {
                        ...
                        val index = view.indexFromRelPos(cellRelPos + XY(x, y))

...
{
    def forMaster(bot: Bot, previousMove: ListBuffer[XY]) {
        val (directionValue, nearestEnemyMaster, nearestEnemySlave) = analyzeViewAsMaster(bot, previousMove)
    ...
    /** Analyze the view, building a map of attractiveness for the 45-degree directions and
      ...
      */
    def analyzeViewAsMaster(bot: Bot, previousMove: ListBuffer[XY]) = {
        var view = bot.view
        ...
        //view.aStarPathfind(cellWeights, bot)
                  ...
                  if(cellRelPos.isNonZero) {
                    ...
                    case 'b' => // bad beast: dangerous, but only if very close
                        for (j <- 0 until cellCount) {
                          ...
                          val stepDistance = cellRelPos.stepsTo(pos)
                          if (pos.isNonZero && stepDistance != 0) {
                            if (stepDistance < 4) cellWeights(j) += 40000 / stepDistance
                            else cellWeights(j) += 5000 / stepDistance
                          }
                        }
            ...
            if (cells.contains('P') || cells.contains('B')) {
                ...
                if(found)
                {
                        ...
                        {
                            bot.previousMove = bot.previousMove :+ path(path.size - 1)
            ...
            {
                bot.log("nzn")
                if(bot.previousMove.size > 0)
                {
                    bot.log("kazkas")
                    direction45 = bot.previousMove(bot.previousMove.size - 1).toDirection45
                }
            }
        ...
        def findClosestThings(cellCount: Integer, cells: String, view: View, weights: Array[Double], bot: Bot): (XY, Option[XY], Option[XY]) = {
        ...
        for (i <- 0 until cellCount) {
            if (cells(i) == 'P' || cells(i) == 'B') {
                ...
                if (cellRelPos.isNonZero) {
                    ...
                    if(testWeight < 400000 && bot.energy > 2000)
                    {
                        distances(i) -= stepDistance / 2
                    }
                    if(testWeight < 100000 && bot.energy < 2000)
                    {
                        distances(i) -= stepDistance / 2
                    }
    ...
    // Returns true if the cell is not blocked else false 
        if ( colums(index_check) != 'b' || colums(index_check) != 'w')
        {
            (true) 
        }
        else
...
class ControlFunctionFactory {
    var previousMove = ListBuffer[XY]()
    def create = (input: String) => {
        ...
        opcode match {
            case "React" =>
                ...
                if( bot.generation == 0 ) {
                    ControlFunction.forMaster(bot, previousMove)
                } else {

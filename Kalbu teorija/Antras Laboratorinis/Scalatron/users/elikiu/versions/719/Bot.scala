...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        //view.aStarPathfind(cellWeights, bot)
            ...
            if (cells.contains('P') || cells.contains('B')) {
                ...
                var (SecondClosestFood, innerMasterSec, innerSlaveSec) = findClosestThings(cellCount, cells, view)
    ...
    def findClosestThings(cellCount: Integer, cells: String, view: View): (XY, Option[XY], Option[XY]) = {
        ...
        var minFilterValue = istances.filter(_ > 0).min
        distance = distance.filter(_ > minFilterValue)
        (minFirst, view.relPosFromIndex(distances.indexOf(distances.min)),
            closestEnemyMaster,
            closestEnemySlave)
    }

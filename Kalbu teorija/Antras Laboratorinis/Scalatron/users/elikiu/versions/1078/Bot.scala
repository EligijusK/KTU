...
object ControlFunction
{
    ...
    def reactAsKamikaze(bot: MiniBot) {
        bot.view.offsetToNearest('m') match {
          case Some(delta: XY) =>
            bot.set("rx" -> delta.x, "ry" -> delta.y)
            if (delta.length <= 3) {
        ...
        bot.view.offsetToNearest('s') match {
          case Some(delta: XY) =>
            ...
            {
                val directionValue = XY[delta.x, delta.y]
                val direction = XY.fromDirection45(directionValue)
                bot.move(direction) 
            }
          case None =>
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        //view.aStarPathfind(cellWeights, bot)
            ...
            if(!cells.contains('P') && !cells.contains('B'))
            {
                var farest = findFarestThing(cellCount, cells, view, bot, cellWeights)
                
                var (path, path_index, found) = aStarPathfind(cells, XY.Zero, view, farest, bot, cellWeights)
                bot.log("kazkas turi vykti")
                if(found)
                {
                        //bot.log(cells)
                        //bot.log(path(path.size - 1).toString)
                        ...
                        if((x != 0 && y != 0) ||(x == 0 && y != 0) || (x != 0 && y == 0))
                        {
                            ...
                            { 
                                ...
                                else if(closed_list(index) == false && isUnBlocked(cells, index))
                                {
                                    ...
                                    var temp_pos = loopingPos + XY(-1, -1)
                                    var temp_index = view.indexFromRelPos(pos)
                                    average_weight = average_weight + weights(temp_index)
                                    
                                    temp_pos = loopingPos + XY(-1, 0)  
                                    temp_index = view.indexFromRelPos(pos)
                                    average_weight = average_weight + weights(temp_index)
                                    
                                    temp_pos = loopingPos + XY(0, -1)  
                                    temp_index = view.indexFromRelPos(pos)
                                    average_weight = average_weight + weights(temp_index)
                                    
                                    temp_pos = loopingPos + XY(0, 0)  
                                    temp_index = view.indexFromRelPos(pos)
                                    average_weight = average_weight + weights(temp_index)
                                    ...
                                    //average_weight = average_weight / 16
                                    var gNew = 0.0
                                    if((x == 0 && y != 0) || (x != 0 && y == 0))
                                    {

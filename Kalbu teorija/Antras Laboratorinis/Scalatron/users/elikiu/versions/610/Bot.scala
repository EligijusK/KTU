...
object ControlFunction
{
    ...
    def analyzeViewAsMaster(bot: Bot) = {
        ...
        //view.aStarPathfind(cellWeights, bot)
            ...
            if (cells.contains('P') || cells.contains('B')) {
                ...
                var (path, found) = aStarPathfind(cells, XY.Zero, view, closestFood, bot)
                if(found)
                {
                    bot.log("rado")
    ...
    def aStarPathfind(cells: String, startingPoint: XY, view: View, destination: XY, bot: Bot) = {
        ...
        var index_list = ListBuffer[Int]()
        for(i <- 0 until cells.length)
        {
            closed_list = closed_list :+ false
            parent = parent :+ XY(-1,-1)
            g = g :+ Float.MaxValue
            f = f :+ Float.MaxValue
            h = h :+ Float.MaxValue
        }
        ...
        parent.updated(index, startingPoint)
                        ...
                        //{
                            ...
                            { 
                                if(pos == destination)
                                {
                                    ...
                                    //parent()
                                    path = tracePath(parent, destination, view)
                                    foundDest = true
                                    break
                                }
                                else if(closed_list(index) == false)
                                {
                                    ...
                                    { 
                                        ...
                                        parent.update(index, loopingPos)
                                        if(!index_list.isEmpty && index_list.contains(index))
                                        {
                                            var update_index = index_list.indexOf(index)
                                        }
    ...
    def tracePath(parent: ListBuffer[XY], dest: XY, view: View) = { 
        ...
        { 
            ...
            index = view.indexFromRelPos(parent(index)) 
        } 
